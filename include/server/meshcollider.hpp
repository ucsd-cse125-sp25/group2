...
private:
  ...
  void LoadFromObj(const std::string& path) {
    ...
    bool ret = tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, (base_dir + "/" + file_name).c_str(), base_dir.c_str());
  ...
  Triangle TransformTriangle(const Triangle& tri, const glm::mat4& mat) const {
    return {
      glm::vec3(mat * glm::vec4(tri.v0, 1.0f)),
  ...
  bool TriangleTriangleIntersect(const Triangle& triA, const Triangle& triB, Contact& contact) {
    glm::vec3 edgesA[3] = {
        triA.v1 - triA.v0,
        triA.v2 - triA.v1,
        triA.v0 - triA.v2
    ...
    if (minOverlap >= std::numeric_limits<float>::max() || minOverlap <= 1e-6f) {
      return false;
    }
    ...
    std::cout << contact.normal.x << " " << contact.normal.y << " " << contact.normal.z << std::endl;
    return true;
  }

  void ProjectTriangleOntoAxis(const Triangle& tri, const glm::vec3& axis, float& min, float& max) {
  ...
  glm::vec3 CalculateContactPoint(const Triangle& triA, const Triangle& triB) {
    ...
    if (PointInTriangle(triA.v0, triB)) contacts.push_back(triA.v0);
    if (PointInTriangle(triA.v1, triB)) contacts.push_back(triA.v1);
    if (PointInTriangle(triA.v2, triB)) contacts.push_back(triA.v2);
    if (PointInTriangle(triB.v0, triA)) contacts.push_back(triB.v0);
    if (PointInTriangle(triB.v1, triA)) contacts.push_back(triB.v1);
    if (PointInTriangle(triB.v2, triA)) contacts.push_back(triB.v2);
