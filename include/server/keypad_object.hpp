...
public:
  ...
  KeypadObject(const OBJECT_ID id, const LEVEL_ID levelNum, const bool isActive,
             ...
             unique_ptr<RigidBody> &rb, vector<Collider *> &cl, const vector<int> &sequence)
      : GameObject(id, levelNum, isActive, originalPosition, tf, rb, cl), correctSequence(sequence),
        unlocked(false), locked(false), clientUsing(-1), opened(false){};
  bool unlocked;
  bool locked;
