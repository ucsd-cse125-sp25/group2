...
bool ClientGameState::init() {
  ...
  while (in.read_row(objectId, objectType, isActive, posX, posY, posZ, rotX, rotY, rotZ, scaleX, scaleY, scaleZ, modelPath, vertShaderPath, fragShaderPath)) {
      if (objectType == "CUBE") {
        ...
        obj->setShader(make_unique<Shader>(vertShaderPath.c_str(), fragShaderPath.c_str()));
...
void ClientGameState::update(int id, Transform* tf) {
  // cout << tf->getPosition().x << " " << tf->getPosition().y << " " << tf->getPosition().z << endl;
