...
void UIManager::make_StartScreen() {
  startScreenUI = make_unique<BaseUI>(0.0f, 0.0f, 2.0f, 2.0f, 0, AnimationInfo(3,6,0.06f), false, false);
  ...
  startButton = make_unique<BaseUI>(0.0f, -0.5f, 0.5f, 0.5f, 0, AnimationInfo(1,2,0.1f), true, true);
  startButton->setTexture(
    BaseUI::loadTexture("../resources/ui/StartButton.png"));
  startButton->setShader(make_unique<Shader>(
      "../resources/shaders/animUi.vert", "../resources/shaders/animUi.frag"));
  startButton->setHoverTexture(BaseUI::loadTexture("../resources/ui/StartButtonHover.png"));
}

void UIManager::update_start(float mouseX, float mouseY, int winWidth, int winHeight, float deltatime) {
...
void UIManager::update_menu(float mouseX, float mouseY, int winWidth, int winHeight, float deltatime) {
...
void UIManager::draw_start() { startScreenUI->draw(); startButton->draw(); }
