  ...
  for (GameObject *obj : this->objects) {
    ...
    if (!obj->isGrounded()) {
      ...
      if (glm::dot(vel, vel) > 0.001f)
        force += 0.5f * this->density * glm::dot(vel, vel) * this->drag * obj->getRigidBody()->getArea() * -1.0f * glm::normalize(vel);
    }
    ...
    for (GameObject *a : objects) {
      for (GameObject *b : objects) {
        ...
        if (aCol->intersects(*bCol, normal, penetration)) {
          ...
          if (aOnTop || bOnTop) {
            if (aOnTop) a->setGrounded(true);
            if (bOnTop) b->setGrounded(true);
          }
          ...
          float restitution = std::min(a_rb->getRestitution(), b_rb->getRestitution());
          ...
          if (v_close < 0 && massSum > 0) {
            ...
            if (!a_rb->isStatic()) a_rb->applyImpulse(-impulse * invMassA);
            if (!b_rb->isStatic()) b_rb->applyImpulse(impulse * invMassB);
          }
          ...
          if (massSum > 0) {
            glm::vec3 correction = std::max(penetration - slop, 0.0f) / massSum * percent * normal;
            if (!a_rb->isStatic())
...
void Physics::MoveObjects(float deltaTime) {
  for (GameObject *obj : this->objects) {
    if (obj->getRigidBody()->isStatic()) continue;
