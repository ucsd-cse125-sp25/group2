...
void Physics::CalculateForces() {
  for (GameObject *obj : this->objects) {
    ...
    if (vel != glm::vec3(0) && !obj->isGrounded())
      force += 0.5f * this->density * glm::dot(vel, vel) * this->drag * obj->getArea() * -1.0f * glm::normalize(vel);
...
void Physics::SolveCollisions(std::vector<Collision> collisions, float deltaTime) {
...
void Physics::Solve(Collision collision, float deltaTime)
{
  ...
  // Normal doesn't make sense, might need to change collider completely for this.
  glm::vec3 normal = glm::normalize(a->getTransform()->getPosition() - b->getTransform()->getPosition());

  float a_v_close = glm::dot(a_vel - b_vel, normal);
  float b_v_close = glm::dot(b_vel - a_vel, -normal);
  glm::vec3 impulse = -(1 + a_rb->getRestitution()) * a_rb->getMass() * a_v_close * normal;
  a_rb->applyImpulse(impulse);
  impulse = -(1 + a_rb->getRestitution()) * a_rb->getMass() * b_v_close * -normal;
...
void Physics::MoveObjects(float deltaTime) {
  for (GameObject *obj : this->objects) {
    ...
    int id = obj->getId();
    float halfHeight = 1;
    if (id == 0) halfHeight = 0.25f;
    else if (id == 1) halfHeight = 0.5f;
    else if (id == 2) halfHeight = 0.625f;
    else if (id == 3) halfHeight = 0.75f;
    
    glm::vec3 vel = rb->getVelocity() 
      + rb->getForce() / rb->getMass() 
      * deltaTime;
