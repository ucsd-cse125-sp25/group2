...
void GameServer::start() {
  ...
  network->setOnJoin([game = game.get(), &select = characterManager, net = network.get()]() {
    if (game->state != Gamestate::GAME) {
        CharacterResponsePacket responsePacket(select->chicken,select->sheep,select->pig, select->cow);
        net->sendToAll(responsePacket);
    }
  });
  network->setOnLeave([game = game.get(), &select = characterManager, net = network.get()](int id) {
    if (game->state != Gamestate::GAME) {
  ...
  while (!packetsList.empty()) {
    ...
    switch (packet->getType()) {
      ...
      case PacketType::CHARACTERSELECT: {
        ...
        characterManager->assignCharacter(characterPacket->character, characterPacket->clientID);
        CharacterResponsePacket responsePacket(characterManager->chicken,characterManager->sheep,characterManager->pig, characterManager->cow);
