...
void Physics::solveCollision(GameObject *a, GameObject *b, Contact contact) {
  // Get rigid bodies
  ...
  if (!b_rb->isStatic()) {
    b->getTransform()->updatePosition(correction);
  }
}
  ...
  for (int i = 0; i < solverIterations; ++i) {
    for (int i = 0; i < objects.size(); ++i) {
      for (int j = i + 1; j < objects.size(); ++j) {
        ...
        MeshCollider *aMCl = a->getMeshCollider();
        MeshCollider *bMCl = b->getMeshCollider();
        if (!aMCl || !bMCl)
          continue;

        Contact contact;
        if (aMCl->intersects(bMCl, contact)) {
...
void Physics::moveObjects(float deltaTime) {
  ...
  for (GameObject *obj : objects) {
    ...
    MeshCollider *mcl = obj->getMeshCollider();
